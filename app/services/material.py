    ...
    for attempt in range(max_retries):
        try:
            ...
            if attempt == max_retries - 1:
                logger.error(f"search videos failed permanently: {str(e)}")
                return []
            delay = base_delay * (2 ** attempt)
            logger.warning(f"search videos failed (attempt {attempt + 1}/{max_retries}): {e}. Retrying in {delay}s...")
            time.sleep(delay)
...
def save_video(video_url: str, save_dir: str = "") -> str:
    ...
    for attempt in range(max_retries):
        try:
            ...
            if attempt == max_retries - 1:
                logger.error(f"failed to download video from Pexels permanently: {e}")
                raise e
            delay = base_delay * (2 ** attempt)
            logger.warning(f"failed to download video (attempt {attempt + 1}/{max_retries}): {e}. Retrying in {delay}s...")
            time.sleep(delay)
...
def download_videos(
    ...
    if source == "pixabay":
        ...
        search_videos = search_videos_hunyuan
