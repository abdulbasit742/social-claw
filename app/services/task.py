...
def generate_script(task_id, params):
    ...
    if not video_script:
        from app.config import config
        if config.app.get("enable_trend_scraper", False):
            try:
                import concurrent.futures
                import asyncio
                from app.services.trend_scraper import get_social_trends
                logger.info("Fetching viral trends from social media...")
                with concurrent.futures.ThreadPoolExecutor() as executor:
                    future = executor.submit(lambda: asyncio.run(get_social_trends()))
                    trends_context = future.result()
                if trends_context:
                    params.video_script_prompt = f"{params.video_script_prompt}\n\n{trends_context}".strip()
...
def get_video_materials(task_id, params, video_terms, audio_duration):
    ...
    if getattr(params, "use_ai_clips", False):
        logger.info("\n\n## generating videos using local HunyuanVideo-1.5 AI")
        video_paths = []
        total_duration = 0.0
        max_clip_duration = params.video_clip_duration or 5
        required_duration = audio_duration * params.video_count
        
        # Format aspect ratio for the skill
        aspect_str = "9:16"
        if params.video_aspect:
            raw_aspect = getattr(params.video_aspect, "value", str(params.video_aspect))
            if "16:9" in raw_aspect:
                aspect_str = "16:9"
            elif "1:1" in raw_aspect:
                aspect_str = "1:1"
            else:
                aspect_str = "9:16"
                
        try:
            from skills.ai_clip import ai_clip
        except ImportError as ie:
            logger.warning(f"Could not import skills.ai_clip: {ie}. Falling back to default downloader.")
            ai_clip = None
            
        for idx, term in enumerate(video_terms):
            if total_duration >= required_duration:
                logger.info(f"Generated enough clips ({total_duration}s >= {required_duration}s), stopping.")
                break
                
            logger.info(f"Generating AI clip {idx+1}/{len(video_terms)} for term: '{term}'")
            saved_path = ""
            if ai_clip:
                try:
                    saved_path = ai_clip(
                    ...
                    logger.error(f"Error calling ai_clip skill for '{term}': {ex}")
                    
            if saved_path and os.path.exists(saved_path):
                video_paths.append(saved_path)
                total_duration += max_clip_duration
            else:
                logger.warning(f"Failed to generate AI clip for '{term}', falling back to {params.video_source} download.")
                try:
                    ...
                    if fallback_videos:
                        video_paths.extend(fallback_videos)
                        total_duration += max_clip_duration
                except Exception as fe:
                    logger.error(f"Fallback download failed for '{term}': {fe}")
                    
        if not video_paths:
            sm.state.update_task(task_id, state=const.TASK_STATE_FAILED)
            logger.error("failed to generate or download any video materials.")
            return None
        return video_paths
