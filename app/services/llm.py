...
def _generate_response_core(prompt: str) -> str:
    try:
        content = ""
        llm_provider = config.app.get("llm_provider", "openai")
        logger.info(f"llm provider: {llm_provider}")
        
        if llm_provider == "chatgpt_browser":
            from app.services.chatgpt_browser import ask_chatgpt
            import asyncio
            import concurrent.futures
            # Run the async Playwright code in a separate thread to prevent event loop collision
            with concurrent.futures.ThreadPoolExecutor() as executor:
                future = executor.submit(lambda: asyncio.run(ask_chatgpt(prompt)))
...
def _generate_response(prompt: str) -> str:
    ...
    for attempt in range(max_retries):
        try:
            content = _generate_response_core(prompt)
            if isinstance(content, str) and content.startswith("Error:"):
                raise ValueError(content)
            return content
        except Exception as e:
            if attempt == max_retries - 1:
                logger.error(f"LLM request failed permanently after {max_retries} attempts: {e}")
                return f"Error: {e}"
...
2. each search term should consist of 1-2 words. Use concrete, visually descriptive nouns and verbs (e.g., 'office meeting', 'typing keyboard', 'money coins', 'handshake business', 'talking presentation', 'arrow up growth') rather than abstract concepts to ensure high-quality matching Pexels stock video footage.
...
SOCIAL_PLATFORMS = {
    ...
    "linkedin": {"title_max": 100, "caption_max": 3000, "hashtag_count": 5},
    "telegram": {"title_max": 100, "caption_max": 1000, "hashtag_count": 3},
}
...
3. "title": a catchy, highly curiosity-driven hook to maximize click-through rate, using relevant keywords, at most {spec['title_max']} characters.
...
5. "hashtags": a JSON array of exactly {spec['hashtag_count']} strings. Each must start with "#", contain no spaces. For business, startups, and success topics, prioritize relevant tags like #entrepreneur #startup #business #success #motivation.
