...
def _safe_write_videofile(clip, output_file: str, codec: str, **kwargs):
    try:
        clip.write_videofile(output_file, codec=codec, **kwargs)
    except PermissionError as e:
        if "TEMP_MPY" in str(e) and os.path.exists(output_file):
            logger.warning(f"Ignored MoviePy Windows temp file locking bug: {e}")
        else:
            raise
    except Exception as e:
        if "PermissionError" in str(e) and "TEMP_MPY" in str(e) and os.path.exists(output_file):
            logger.warning(f"Ignored wrapped MoviePy Windows temp file locking bug: {e}")
        else:
            raise
...
def _fallback_write_videofile(clip, output_file: str, failed_codec: str, reason: str, **kwargs):
    ...
    """
    _safe_write_videofile(clip, output_file, codec=_DEFAULT_VIDEO_CODEC, **kwargs)
...
def _write_videofile_with_codec_fallback(clip, output_file: str, codec: str, **kwargs):
    ...
    硬件编码器是否可用不仅取决于 FFmpeg，还取决于显卡、驱动 and 当前运行环境。
    生成任务不能因为高级编码器不可用而整体失败，所以这里把回退集中处理。
    """
    effective_codec = _get_effective_video_codec(codec)
    try:
        _safe_write_videofile(clip, output_file, codec=effective_codec, **kwargs)
