...
def convert_wav_to_mp3(wav_path: str, mp3_path: str) -> bool:
    ...
    logger.info(f"Transcoding WAV to MP3: {wav_path} -> {mp3_path}")
    result = subprocess.run(command, capture_output=True, text=True, check=False)
    if result.returncode != 0:
        logger.error(f"FFmpeg transcode failed: {result.stderr or result.stdout}")
        return False
...
def pyttsx3_tts(
    ...
    use_temp_wav = voice_file.lower().endswith(".mp3")
    sapi_file = voice_file + ".wav" if use_temp_wav else voice_file
    ensure_file_path_exists(sapi_file)

    logger.info(f"Generating offline SAPI TTS via pyttsx3, rate multiplier={voice_rate}, output={sapi_file}...")
    try:
        try:
            import pythoncom
            pythoncom.CoInitialize()
        except Exception as com_err:
            logger.debug(f"pythoncom.CoInitialize failed: {com_err}")
            
        import pyttsx3
        engine = pyttsx3.init()
        
        try:
            rate = int(165 * float(voice_rate))
        except:
            rate = 165
        engine.setProperty("rate", rate)
        
        try:
            vol = float(voice_volume)
            if vol <= 0.0:
                vol = 1.0
        except:
            vol = 1.0
        ...
        for v in voices:
            v_id = getattr(v, "id", "")
            v_lang = str(getattr(v, "languages", ""))
            v_name = getattr(v, "name", "")
            if "en" in v_id.lower() or "en" in v_lang.lower() or "english" in v_name.lower():
                selected_voice = v_id
                break
        if selected_voice:
            engine.setProperty("voice", selected_voice)
            
        engine.save_to_file(text, sapi_file)
        engine.runAndWait()
        
        time.sleep(0.5)

        if not os.path.exists(sapi_file) or os.path.getsize(sapi_file) <= 0:
            logger.error(f"pyttsx3 failed to generate audio file: {sapi_file}")
            return None

        if use_temp_wav:
            success = convert_wav_to_mp3(sapi_file, voice_file)
            try:
                os.remove(sapi_file)
            except Exception as rm_err:
                logger.warning(f"Could not remove temp wav file: {rm_err}")
            if not success:
                return None

        duration = get_audio_duration(voice_file)
        logger.info(f"pyttsx3 completed. Output file: {voice_file}, duration: {duration:.2f}s")
        ...
        logger.error(f"pyttsx3_tts failed: {e}")
        return None
...
def tts(
    ...
    result = None
    try:
        result = _tts_internal(text, voice_name, voice_rate, voice_file, voice_volume)
    except Exception as e:
        logger.error(f"Selected TTS engine failed: {e}")
        result = None

    if result is None:
        logger.warning(f"Selected TTS engine ({voice_name}) failed or timed out! Falling back to offline pyttsx3...")
        result = pyttsx3_tts(text, voice_file, voice_rate, voice_volume)
...
def get_audio_duration(target: Union[str, SubMaker]) -> float:
    ...
    elif isinstance(target, str) and target.lower().endswith(".wav"):
        try:
            import wave as _wave
            with _wave.open(target, "rb") as wf:
                frames = wf.getnframes()
                rate = wf.getframerate()
                duration = frames / float(rate)
                logger.info(f"WAV duration: {duration:.2f}s ({frames} frames @ {rate}Hz)")
                return duration
        except Exception as e:
            logger.error(f"Failed to get audio duration from WAV: {str(e)}")
            return 0.0
    else:
