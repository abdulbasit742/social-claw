...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help="use HunyuanVideo-1.5 AI b-roll clips instead of stock video APIs",
    )
    parser.add_argument(
        "--video-materials",
        default="",
    ...
    parser.add_argument(
        "--custom-audio-file",
        default=None,
        help="path to a pre-generated audio file; skips TTS entirely",
    )
    parser.add_argument(
        "--video-concat-mode",
...
def build_video_params(args: argparse.Namespace) -> VideoParams:
    ...
    params_kwargs = {
        ...
        "use_ai_clips": args.use_ai_clips,
    }
...
def run_cli(argv: Sequence[str] | None = None) -> int:
    ...
    if result and getattr(args, "upload_youtube", False) and args.stop_at == "video":
            ...
            for video in videos:
                try:
                    url = youtube_upload.upload_video(
                        ...
                        script=script,
                        privacy_status=getattr(args, "youtube_privacy", "unlisted")
                    )
                    if url:
                        logger.success(f"YouTube Upload URL: {url}")
                        result["youtube_url"] = url
                except Exception as upload_err:
                    logger.error(f"Failed to upload {video} to YouTube: {upload_err}")

    print(json.dumps({"task_id": task_id, "result": result}, ensure_ascii=True))
    return 0
